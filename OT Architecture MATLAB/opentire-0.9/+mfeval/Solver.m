...
% NK: Modified by NK with bug fixes and compatibility with mftire 1.0.0
    ...
    methods
        ...
        function out = fullSteadyState(obj, tirParams, inputs, useMode)%#codegen
            ...
            % Call doExtras
            [Re, omega, rho, Rl, a, b, Cx, Cy, Cz, sigmax, sigmay, instKya] = obj.doExtras(tirParams, postProInputs, forcesAndmoments, varinf, modes);
        ...
        function out = fullCPI(obj, tirParams, inputs, userUseMode, userDynamics)%#codegen
            ...
            % Call doExtras
            [~, ~, rho, Rl, ~, ~, ~, ~, ~, ~, ~] = obj.doExtras(tirParams, postProInputs, forcesAndmoments, varinf, modes);
...
% NK: Changed so parameters are always clamped to their limits even if
% NK: useLimitsCheck is off, to prevent numerical problems.
% NK: useLimitsCheck enables low-speed model
            
            % Minimum Speed
            VXLOW = tirParams.VXLOW;

            % useLimitsCheck enables low-speed model
            if useLimitsCheck
                % Turn slip modifications
% NK: Removed, this is handled in phi computation.
% NK: In any case, this should not be inside useLimitsCheck section.
% NK: removed:  phit = phit.*cos(alpha); % Empirically discovered
...
% NK: Does this match TNO?  This reduction probably be applied to the *net* 
% NK: tire spin phi, computed later.
% NK: In some ways this defeats purpose of turn slip calcs, since they are
% NK: primarily for low-speed maneuvers.
...
% NK: This is odd, does this match TNO? Is it found in any reference documents?
% NK: Seems like sqrt(Vcx^2 + Vsy^2) would make more sense
 ...
 % NK: Now, useLimitsCheck just affects whether warning messages are displayed
 % NK: and whether low-speed model is applied
            % Warn if anything is out of range.
            if (useLimitsCheck)
                if any(isLowSlip)
...
% NK: Changed to handle MF5.2 NOMPRES == 0
            dpi = 0;
            if (tirParams.NOMPRES > 0)
            ...
            if ncolumns > 7 && sum(inputs(:, 8)) ~= 0
                ...
                omega = inputs(:,8);
% NK: Moved here inside 'if' to avoid a second call
                [Re, ~, ~] = obj.calculateRe(tirParams, postProInputs, dpi);
            else
                % If omega is NOT one of the inputs: Estimate omega
% NK: Now uses the Re return value
                [Re, ~, omega] = obj.calculateRe(tirParams, postProInputs, dpi);
            end % if omega
            
            % Save omega
            postProInputs.omega = omega;
% NK: moved up [Re, ~, ~] = obj.calculateRe(tirParams, postProInputs, dpi);
...
% NK: The explanation for the empirically discovered multiplication of phit by cos(alpha) is
% NK: that phi is computed using Vcx_prime, not V or Vc.  The difference is a factor of
% NK: cos(alpha). This is shown in Eqs. 6 and 7 "Using the PAC2002 Tire Model".
% NK: Also, contrary to comments above, the book (at least my book) uses Vc, not Vcx)

% NK: Changed to use make_nonzero
                Vcx_prime = make_nonzero(Vcx, epsilonv);
                
% NK: I don't quite understand the isLowSpeed checks here
% NK: removed   % Speed limits (avoid zero speed)
% NK: removed   %Vc_prime = Vcx; % From the book
% NK: removed   Vc_prime = V; % From the Equation Manual
% NK: The explanation for the empirically discovered adjustment of phit by cos(alpha) is
% NK: that phi is computed using Vcx_prime, not V or Vc.  The difference is a factor of
% NK: cos(alpha). This is shown in Eqs. 6 and 7 "Using the PAC2002 Tire Model".
% NK: (also, contrary to comments above, the book uses Vc, not Vcx)
% NK: removed                isLowSpeed = abs(Vcx) < tirParams.VXLOW;
% NK: removed   signVcx = sign(Vcx(isLowSpeed));
% NK: removed   signVcx(signVcx == 0) = 1;
% NK: removed   Vc_prime(isLowSpeed) = real(tirParams.VXLOW * signVcx); % Singularity protected velocity, text on Page 184

% NK: Spin slip is defined in Eqs. 2.15-2.18 of the book, in the "adapted SAE" system.
% NK: Fig. 2.6 on page 66 shows that camber increases the magnitude of 
% NK: turnslip when the tire is cambered towards the center of the curve.
% NK: A tire in a right turn as shown in 2.6 will have negative path curvature
% NK: and a positive camber angle. By the right hand rule in the ISO system,
% NK: a positive wheel rotation with positive camber will induce a negative angular
% NK: velocity about the Z axis:
% NK: 
% NK:   psi_dot_omega = wheel_omega * -sin_gamma;
% NK: 
% NK: This corresponds to Eq. (7) of [3], which shows the negative sign
% NK: in front of the (1-eps_gamma) term.

                psi_dot_omega = omega .* -sin(gamma); % psi_dot induced by wheel rotation
                psi_dot = phit .* Vcx_prime; % psi_dot from path curvature Rearrange [Eqn (4.75) Page 183 - Book]
                
                phi = (1./Vcx_prime) .*(psi_dot + (1-epsilong).*psi_dot_omega); % [Eqn (4.76) Page 184 - Book]
                
% NK: Suggestion for phit/phi low speed reduction:
% NK: Instead of scaling phit earlier, scale phi here:
% NK:           phi = phi .* reductionLinear;
...
% NK: The Matlab sign() function does the wrong thing when its argument is zero.
% NK: This might cause discontinuities; the new SGN() function is used throughout this file instead.
            if useAlphaStar
                alpha_star = tan(alpha).*SGN(Vcx); % [Eqn (4.E3) Page 177 - Book]
...
% NK: new make_nonzero() function.
            Vc_prime = make_nonzero(Vc, epsilonv); % [Eqn (4.E6a) Page 178 - Book]
...
% NK: make_nonzero()
            Bx = Kxk./make_nonzero(Cx.*Dx, epsilonx); % (4.E16)
            ...
            Ex = (PEX1 + PEX2.*dfz + PEX3.*dfz.^2).*(1 - PEX4.*SGN(kappax)).*LEX; % (<=1) (4.E14)
            
            % Limits check
% NK: Changed to always limit E, but warn only if useLimitsCheck
            if(useLimitsCheck)
                if(any(Ex > 1))
            ...
            %    Fx0(isLowSpeed) = Dx.*(1-internalParams.reductionSmooth)*SGN(kappax)+ Fx0.*internalParams.reductionSmooth;
...
% NK: make_nonzero()
                Kya_prime = make_nonzero(Kya, epsilonk); % (4.E39)
% NK: Renamed to Kya0_prime instead of o
                Kya0_prime = make_nonzero(Kya0, epsilonk); % epsilonk is a small factor added to avoid the singularity condition during zero velocity (equation 308, CarMaker reference Manual).
                
                CHyp = PHYP1; % (>0) [Eqn (4.85) Page 186 - Book]
                DHyp = (PHYP2 + PHYP3.*dfz).*SGN(Vcx); % [Eqn (4.86) Page 186 - Book]
...
% NK: This should be Kya_prime, not Kya0_prime
% NK: was       BHyp = KyRp0./(CHyp.*DHyp.*Kya0_prime); %[Eqn (4.88) Page 186 - Book]
                BHyp = KyRp0./(CHyp.*DHyp.*Kya_prime); %[Eqn (4.88) Page 186 - Book]
                SHyp = DHyp.*sin(CHyp .* atan(BHyp.*R0.*phi - EHyp.*(BHyp.*R0.*phi - atan(BHyp.*R0.*phi)))).*SGN(Vcx); % [Eqn (4.80) Page 185 - Book]
...
% NK: make_nonzero()
            ...
            if tirParams.FITTYP == 6
                ...
                % MF6.1 and 6.2 equatons
% NK: make_nonzero()
                SHy = (PHY1 + PHY2.*dfz).* LHY + ((Kyg0 .*gamma_star - SVyg)./make_nonzero(Kya, epsilonk)).*zeta0 +zeta4 -1; % (4.E27)
            end
...
% NK: use SGN()
            Ey = (PEY1 + PEY2.*dfz).*(1 + PEY5.*gamma_star.^2 - (PEY3 + PEY4.*gamma_star).*SGN(alphay)).*LEY; % (<=1)(4.E24)
...
% NK: make_nonzero
            By = Kya./make_nonzero(Cy.*Dy, epsilony); % (4.E26)
            ...
            %    % Low speed model and Linear Transients
% NK: Code in this comment should probably use make_nonzero(), but I am not sure the intent.
        ...
        function [Mz0, alphar, alphat, Dr, Cr, Br, Dt, Ct, Bt, Et, Kya_prime, Gyk, SVyk] = calculateMz0(obj, tirParams, postProInputs, internalParams, modes, starVar, primeVar, incrVar, Kya, ~, SHy, SVy, By, Cy)%#codegen
...
% NK: make_nonzero()
            ...
            % Dt0 = Fz.*(R0./Fz0_prime).*(QDZ1 + QDZ2.*dfz).*(1 - PPZ1.*dpi).* LTR.*SGN(Vcx); % (4.E42)
            ...
            % Paper definition:
% NK: gamma_star should be used in place of gamma for all the paper equations.
            Dt = (QDZ1 + QDZ2.*dfz).*(1 - PPZ1.*dpi).* (1 + QDZ3.*gamma_star + QDZ4.*gamma_star.^2).*Fz.*(R0./Fz0_prime).*LTR.*zeta5; % (A60)
            ...
            % Paper definition:
% NK: gamma_star should be used in place of gamma for all the paper equations (paper wasn't written with gamma_star in model)
            Bt = (QBZ1 + QBZ2.*dfz + QBZ3.*dfz.^2).*(1 + QBZ4.*gamma_star + QBZ5.*abs(gamma_star) ).*LKY./LMUY_star; %(> 0) (A58)
            Ct = QCZ1; % (> 0) (4.E41)
...
% NK: Changed to always limit E, but warn only if useLimitsCheck
            ...
            t0 = Dt.*cos(Ct.*atan(Bt.*alphat - Et.*(Bt.*alphat - atan(Bt.*alphat)))); %t(aplhat)(4.E33)
            
            
% NK: In the following code, the idea is to compute Gyk and Gyk0, which are values at kappa and at kappa == 0.
% NK: The influence of gamma on anything but turnslip is not changed.  In the book, the '0' of Gyk0 means that the evaluation
% NK: is done with kappa=0.  And gamma=phit=0 really means "no net turnslip" or phi=0. Comments changed below.
            % Evaluate Fy0 at kappa = 0, gamma = 0, and phi = 0
            modes_sub0 = modes;
            modes_sub0.useTurnSlip = false;
            
            postProInputs_sub0 = postProInputs;
            postProInputs_sub0.gamma = zeros(size(Fz));
            postProInputs_sub0.phit = zeros(size(Fz));
            postProInputs_sub0.phi = zeros(size(Fz));
            
            starVar_sub0 = starVar;
            starVar_sub0.gamma_star = zeros(size(Fz));
            
% NK: Mz0_prime isn't used or returned; these lines can be commented out
            %[Fy0_sub0, ~, ~, ~, ~, ~, ~, ~] = obj.calculateFy0(tirParams, postProInputs, internalParams, modes_sub0, starVar_sub0, primeVar, incrVar);
            
% NK: Name changed to Mz0
            %Mz0_prime = -t0.*Fy0_sub0; % gamma=phi=0 (4.E32)
            ...
            if useTurnSlip
                ...
                epsilong = internalParams.epsilong;
% NK: net turnslip phi should be used, not phit
% NK: was       phit = postProInputs.phit;
                phi = postProInputs.phi;
...
% NK: typo: was QDZ8.*QDZ9 and parentheses was after .*abs(gamma), not before.
% NK: bug       Kzgr0 = Fz.*R0.*(QDZ8 + QDZ9.*dfz + (QDZ10 + QDZ11.*dfz.*abs(gamma))).*LKZC; %[Eqn (4.99) Page 187 - Book]
                Kzgr0 = Fz.*R0.*(QDZ8 + QDZ9.*dfz + (QDZ10 + QDZ11.*dfz).*abs(gamma)).*LKZC; %[Eqn (4.99) Page 187 - Book]
% NK: Division needs singularity protection; use make_nonzero(CDrp.*DDrp.*(1-epsilong), 1e-6) or something
                BDrp = Kzgr0./(CDrp.*DDrp.*(1 - epsilong)); % Eqn from the manual
% NK: net turnslip phi should be used, not phit
                Drp = DDrp.*sin(CDrp.*atan(BDrp.*R0.*(phi))); % Eqn from the manual
% NK: was       Drp = DDrp.*sin(CDrp.*atan(BDrp.*R0.*(phit))); % Eqn from the manual
                
                [~,Gyk,SVyk] = obj.calculateFy(tirParams, postProInputs, internalParams, modes, starVar, incrVar, Fy0, muy);
                
% NK: net turnslip phi should be used, not phit
% NK: The book is very clear this parameter is supposed to be phit and not phi, BUT I found the equation
% NK: listed using phi and not phit in both "Using the PAC2002 Tire Model" and "Study on improving the MFSwift tyre model"
% NK: This needs to be checked against TNO.
                Mzp90 = Mzp_inf.*(2/pi).*atan(QCRP2.*R0.*abs(phi)).*Gyk; % [Eqn (4.103) Page 188 - Book]
% NK: was       Mzp90 = Mzp_inf.*(2/pi).*atan(QCRP2.*R0.*abs(phit)).*Gyk; % [Eqn (4.103) Page 188 - Book]
                
% NK: Clamp acos() parameter to [-1..1] to avoid complex results
                theta7 = min(max(Mzp90 ./ abs(Drp), -1), 1);
                zeta7 = (2/pi).*acos(theta7); % Eqn from the manual
% NK: was       zeta7 = (2/pi).*acos(Mzp90./(abs(Drp))); % Eqn from the manual
                zeta8 = 1 + Drp;
            else
                zeta0 = 1;
                zeta2 = ones(size(Fz));
...
% NK: Two problems here.  First, the book uses cos(alpha_prime), not cos(alpha_star) in the original code.  But the second problem
% NK: is in the book: cos(alpha_prime) is used here and later when Mzr is computed: this means that it is applied twice.
% NK: I believe you spotted this problem and removed the term in the Mzr computation, but that still has a problem:
% NK: the turnslip term Drp in zeta8 doesn't have cos(alpha_prime) applied as it should.
% NK: So the solution is to leave cos(alpha_prime) out from this term (and Drp) and multiply it when Mzr is computed.
% NK: Was:  Dr = Fz.*R0.*((QDZ6 + QDZ7.*dfz).*LRES.*zeta2 + ((QDZ8 + QDZ9.*dfz).*(1 + PPZ2.*dpi) + (QDZ10 + QDZ11.*dfz).*abs(gamma_star)).*gamma_star.*LKZC.*zeta0).*LMUY_star.*SGN(Vcx).*cos(alpha_star) + zeta8 - 1; % (4.E47)
% NK: Book: Dr = Fz.*R0.*((QDZ6 + QDZ7.*dfz).*LRES.*zeta2 + ((QDZ8 + QDZ9.*dfz).*(1 + PPZ2.*dpi) + (QDZ10 + QDZ11.*dfz).*abs(gamma_star)).*gamma_star.*LKZC.*zeta0).*LMUY_star.*SGN(Vcx).*cos(alpha_prime) + zeta8 - 1; % (4.E47)
% NK: Equation (56) of "Using PAC2002 Tire Model" shows this formula without the cos(alpha_prime) term, as below:
            Dr = Fz.*R0.*((QDZ6 + QDZ7.*dfz).*LRES.*zeta2 + ((QDZ8 + QDZ9.*dfz).*(1 + PPZ2.*dpi) + (QDZ10 + QDZ11.*dfz).*abs(gamma_star)).*gamma_star.*LKZC.*zeta0).*LMUY_star.*SGN(Vcx) + zeta8 - 1; % (4.E47)
                
            Br = (QBZ9.*LKY./LMUY_star + QBZ10.*By.*Cy).*zeta6; % preferred: qBz9 = 0 (4.E45)
            Cr = zeta7; % (4.E46)
            
% NK: Note that Mzr0 and Mz0 aren't actually used or output anywhere, so probably could be commented out.
% NK: removed % Mzr0 = Dr.*cos(Cr.*atan(Br.*alphar)).*cos(alpha_prime); % =Mzr(alphar)(4.E36)
% NK: removed % Mz0 = Mz0_prime + Mzr0; % (4.E31)
          Mz0 = [];
...
% NK: The term with QSX6 is suspicious, and may be wrong in both the book and paper.
% NK: Book:   cos(QSX5 .* atan(QSX6 .* Fz ./ Fz0)) .^ 2)
% NK: Paper:  cos(QSX5 .* atan((QSX6 .* Fz ./ Fz0) .^ 2))
% NK: Neither is a typical formulation of the cosine form of the Magic Formula.
% NK: I think the correct definition might be:
% NK:         cos(QSX5 .* atan(QSX6 .* (Fz ./ Fz0) .^ 2))
% NK: This would mean that the misprint in book is that ^2 should be inside the parens, not outside.
% NK: This is a typical cos_magic_formula form with QSX6 = B, as documented in the assignment of QSX6 above

% NK: The formula below uses the Paper definition (as in the original MFeval).
% NK: Can you confirm which version is used in TNO?
        ...
        function [My] = calculateMy(obj, tirParams, postProInputs, ~, Fx, modes)%#codegen
...
% NK: These functions are identical when QSY5,6,7,8 are all zero (which would be the case with a MF5.2 file)
% NK: So this check is not needed:
% NK: removed % Check MF version
% NK: removed if tirParams.FITTYP == 6
% NK: removed   % MF5.2 equations
% NK: removed   My = -R0.*Fz_unlimited.*LMY.*(QSY1 + QSY2.*(Fx./Fz0) + QSY3.*abs(Vcx./V0) + QSY4.*(Vcx./V0).^4); % From the MF-Tyre equation manual
% NK: removed else
% NK: removed   % MF6.1 and MF6.2 equations
                % Paper definition:
                My = -R0.*Fz0.*LMY.*(QSY1 + QSY2.*(Fx./Fz0) + QSY3.*abs(Vcx./V0) + QSY4.*(Vcx./V0).^4 ...
                    +(QSY5 + QSY6.*(Fz_unlimited./Fz0)).*gamma.^2).*((Fz_unlimited./Fz0).^QSY7.*(dpi+1).^QSY8); %(A48)
% NK: (dpi+1) instead if p/pi0
            
            % Backward speed check
            My(Vcx < 0) = -My(Vcx < 0);
% NK: I don't understand this low-speed model at all!
% NK: put under limitsCheck for mftire comparisons
        if (modes.useLimitsCheck)
            % Low speed model (Empirically discovered)
% NK: abs(Vcx) can generate division-by-zero: use make_nonzero().

% NK: Seems like this test will start limiting My values even
% NK: for -VXLOW < speed < VXLOW -- is this intendend?

% NK: need internalParams parameter for epsilonv
            highLimit = VXLOW./make_nonzero(Vcx,1e-6) - 1;
            ...
            if any(idx)
                ...
                % Reduce My values
                My(idx) = My(idx).*sin(reduction*pi/2);
            end % if idx
            
            % Negative SR check
            My(kappa<lowLimit) = -My(kappa<lowLimit);
        end
% NK: end limit check test            
            % Zero speed check
% NK: Won't this case have been handled by low speed checks above?
            My(Vcx == 0) = 0;
            
        end % calculateMy

% NK: SVyk is passed in only for use with MF5.2 equations
        function [Mz,t, Mzr] = calculateMz(obj, tirParams, postProInputs, internalParams, modes, starVar, primeVar, incrVar, alphar, alphat, Kxk, Kya_prime, Fy, Fx, Dr, Cr, Br, Dt, Ct, Bt, Et, Gyk, SVyk)%#codegen
            ...
            % alphar_eq = sqrt(alphar.^2+(Kxk./Kya_prime).^2.*kappa.^2).*SGN(alphar); % (4.E78)
            % alphat_eq = sqrt(alphat.^2+(Kxk./Kya_prime).^2.*kappa.^2).*SGN(alphat); % (4.E77)
            ...
            alphar_eq = atan(sqrt(tan(alphar).^2+(Kxk./Kya_prime).^2.*kappa.^2)).*SGN(alphar); % (A54)
            alphat_eq = atan(sqrt(tan(alphat).^2+(Kxk./Kya_prime).^2.*kappa.^2)).*SGN(alphat); % (A55)
            s = R0.*(SSZ1 + SSZ2.*(Fy./Fz0) + (SSZ3 + SSZ4.*dfz).*gamma).*LS; % (A56)
% NK: cos(alpha_prime) term was missing.
            Mzr = Dr.*cos(Cr.*atan(Br.*alphar_eq)).*cos(alpha_prime); % (4.E75)
...
% NK: phi and phit should both be set to zero.
            postProInputs_sub0.phi = zeros(size(gamma));
            postProInputs_sub0.phit = zeros(size(gamma));
...
% NK: muy_sub0 not used any more
% NK: was   [Fy0_sub0, muy_sub0, ~, ~, ~, ~, ~, ~] = obj.calculateFy0(tirParams, postProInputs_sub0, internalParams_sub0, modes, starVar_sub0, primeVar, incrVar);
            [Fy0_sub0, ~, ~, ~, ~, ~, ~, ~] = obj.calculateFy0(tirParams, postProInputs_sub0, internalParams_sub0, modes, starVar_sub0, primeVar, incrVar);
            
% NK: removed % Evaluate Gyk with gamma = 0 and phit  = 0
% NK: removed [~,Gyk_sub0, SVyk] = obj.calculateFy(tirParams, postProInputs_sub0, internalParams_sub0, modes, starVar, incrVar, Fyo_sub0, muy_sub0);
% NK:       assert(all(Gyk_sub0 == Gyk));

            % Note: in the above equation starVar is used instead of
            % starVar_sub0 beacuse it was found a better match with TNO
            
% NK: When calculateFy is called with starVar instead of starVar_sub0, then then the returned Gyk_sub0 always == Gyk
% calculated normally using gamma.  This means that the call above isn't needed.  SVyk is only used for MF5.2,
% and the values to be used are the ones calculated normally (not with gamma=phi=0).
% So instead of this call, Gyk and SVyk are as computed in calculateFy0() and passed to this function as parameters.
            Fy_prime = Gyk.*Fy0_sub0; % (4.E74)
            
            t = Dt.*cos(Ct.*atan(Bt.*alphat_eq - Et.*(Bt.*alphat_eq - atan(Bt.*alphat_eq)))).*cos(alpha_prime); % (4.E73)
            
% NK: Are we sure that 's' above isn't also scaled by LFZO?
            t = t*LFZO;
            ...
            % Low speed model
% NK: I am not sure, but I think maybe the low speed reduction is applied twice to
% NK: some or all of Mz: once to the forces, and again to the trail: only one or the
% NK: other should be scaled.  I haven't looked carefully in this code to make sure this is OK.
% NK: For example if Fy and/or SVyk have been low-speed-scaled then it will be applied a second time
% NK: when Mz is computed from the scaled 't' below.
            isLowSpeed = modes.isLowSpeed;
            if any(isLowSpeed)% Line for Simulink
...
% NK: Added this check for Q_V1 == 0
                % If no omega^2 model term, then we have a quick answer
                % (this can be cleaned up)
                if (Q_V1 == 0)
                    Romega = R0*Q_RE0;
                    Re = Romega - (Fz0./Cz) .* ( DREFF .*atan( BREFF.*(Fz_unlimited./Fz0)) + FREFF.*(Fz_unlimited./Fz0));
                    omega = real((kappa_unlimited.*Vcx+Vcx)./Re); % [Eqn (2.5) Page 65 - Book]
                    return;
                end
                
% NK: This was missing a max(), and the loop would exit when any of the values had converged, not all of them.
% NK: As a result the tolerance can be reduced, tolerance below works well.
% NK: Consider using solve_secant() below, it should converge faster.
% NK: Also set Re_old to zeros to guarantee we enter the loop the first time.
% NK: Also, you can factor out values that don't change during the loop for better performance.
% NK: See mftire.m compute_rolling_radius_and_omega()
                Re_old = zeros(size(Vcx));
                Re_tol = 1e-7/2;
% NK: was       while (abs(Re_old - Re)>1e-9)
                while (max(abs(Re_old - Re)) > Re_tol)
                    Re_old = Re;
...
% NK: Using unlimited Fz
            ...
            fcorr = (1 + speed_effect - fx_effect - fy_effect) .* pressure_effect;
...
% NK: Handled linear case
            ...
            rho_z(Fz==0) = 1e-6;
% NK: This is incorrect: the actual stiffness is the derivative of that quadratic computed above.
% NK: See mftire.m compute_rho_quadratic() for how this can be done (also works for the MF6.2 equations)
...
% NK: Using unlimited Fz
            ...
            % For 6.1.1 and above we must find an iterative solution
            % because of the coupling of loaded_radius/rho, Fy, gamma
            % and the quadratic coefficients.
            lr_v0 = ones(size(omega))*tirParams.UNLOADED_RADIUS; % loaded radius at zero velocity
            Fz_tol = tirParams.FNOMIN * 1e-6;
            Rl = solve_secant(@delta_fz_func, lr_v0 * 0.95, lr_v0, Fz_tol);
            
            % Avoid division between zero
            rho_z(forcesAndmoments.Fz==0) = 1e-6;
            
            % Vertical stiffness (Spring)
% NK: See mftire.m compute_rho_quadratic() for a good way to compute the true stiffness
            Cz = Fz./rho_z;
            
            function delta_fz = delta_fz_func(try_radius)
              [try_Fz, rho_z] = obj.calculateFz62(tirParams, gamma, omega, Romega, dpi, try_radius, Fx, Fy);
              delta_fz = try_Fz - Fz;
            end
...
% NK: fminsearch is quite slow: it doesn't use Newton-Raphson.
% NK: Faster to put this assignment outside the for loop.
...
%             
%             % Vertical stiffness (Spring)
% % NK: See mftire.m compute_rho_quadratic() for a good way to compute stiffness
%             Cz = forcesAndmoments.Fz./rho_z;
%             
        end % calculateRhoRl62
        
        function [a, b, NCz] = calculateContactPatch(~, tirParams, postProInputs, dpi, useLimitsCheck)%#codegen
...
% NK: Disable bottoming model if no limits checking
            Fz0 = tirParams.FNOMIN	; %Nominal wheel load
            if (useLimitsCheck)
              %[DIMENSION]
            ...
            if tirParams.FITTYP == 6
                ...
                % MF6.1 and 6.2 equatons
% NK: REVIEW: Appendix 3 of [1], Eq (A3.7, A3.8) says that
% NK: the contact patch dimensions are best modeled as a function
% NK: of rho rather than the estimate Fz_unlimited./NCz which
% NK: uses the constant stiffness NCz
% NK: 
% NK: Which to use depends on the formulas used to perform the fitting,
% NK: though the results obtained either way should be quite close.

% NK: Error in previous formulation: NCz should be used, not (NCz/R0)
                ss = Fz_unlimited./NCz;
                %ss = rho_z;

                a = R0.*(Q_RA2.*ss + Q_RA1.*sqrt(ss)); %[Eqn (9) Page 3 - Paper]
                b = w.*(Q_RB2.*ss + Q_RB1.*(ss).^(1/3)); %[Eqn (10) Page 3 - Paper]
            end
...
% NK: This isn't a very accurate way to estimate the cornering stiffness, if the
% NK: values being evaluated are not correlated and/or far apart.  The best (but slow)
% NK: way to do this is to simply call mfeval() twice, the second time with perturbed alpha:
% NK: delta = 0.1*pi/180;
% NK: OUTPUT = mfeval(INPUT);
% NK: INPUT_PLUS = INPUT;
% NK: INPUT_PLUS(:,3) = INPUT_PLUS(:,3) + delta;
% NK: OUTPUT_PLUS = mfeval(INPUT_PLUS);
% NK: OUTPUT(:,29) = (OUTPUT_PLUS(:,3) - OUTPUT(:,3) ./ delta;
        ...
        function [Fz, rho_z] = calculateFz62(~, tirParams, gamma, omega, Romega, dpi, Rl, Fx, Fy)
            ...
            % the MF-Tyre/MF-Swift 6.2 equation manual
            % Document revision: 20130706
            
% NK: The formulas in the following paper are implemented below.
% NK: [4] R.H.M.T. van der Hofstad (2010)  Master's Thesis
% NK: 'Study on improving the MFSwift tyre model'
% NK: http://www.mate.tue.nl/mate/pdfs/11495.pdf
...
% NK: bug was: Sfyg = (Q_FYS1 + Q_FYS2.*(Rl./Romega) + Q_FYS3.*(Rl./Romega).^2) .* gamma;
            Sfyg = (Q_FYS1 + Q_FYS2.*(Rl./R0) + Q_FYS3.*(Rl./R0).^2) .* gamma;
...
% NK: Changed this code to reflect the document indicated above.
% NK: The original code had a load discontinuity at Rl = Romega;
% NK: load would drop from non-zero to zero suddenly (as indicated in the paper above)
            abs_gamma = abs(gamma);
            htg = (rtw/2) * tan(abs_gamma);
        
            % loaded_radius > R_omega
            rho_zg = zeros(size(Rl));
            
            sel1 = ((rho_zfr < 0) & (abs_gamma > 0));
            val1 = (htg + max(rho_zfr, -htg)).^2 ./ (4 .* htg);
            rho_zg(sel1) = val1(sel1);

            % loaded_radius <= R_omega
            sel2 = ((rho_zfr >= 0) & (abs_gamma > 0));
            val2 = ( ( ((Q_CAM1.*Rl     + Q_CAM2.*Rl.^2     ).*abs_gamma).^2 - Q_CAM3.*abs_gamma.*Rl     ) .* htg ) ...
                ./ ( ( ((Q_CAM1.*Romega + Q_CAM2.*Romega.^2 ).*abs_gamma).^2 - Q_CAM3.*abs_gamma.*Romega ) .* 4 );
            rho_zg(sel2) = val2(sel2);
            
            % Vertical deflection
            rho_zfr = max(rho_zfr, 0);
            ...
            speed_effect = Q_V2 .*(R0./V0).*abs(omega);
            fx_effect = ((Q_FCX .* Fx)./Fz0).^2;
% NK: Added comment below.
            % IMPORTANT NOTE: In the paper [4], Q_FCY2 is the exponent of
            ...
            % Another possibility for this and SFyg is to use (1+rho/R0).
            % 
            %Q_FCY_ = Q_FCY_ * POW(rho*inv_R0, abs(t.Q_FCY2)); % Eq. 5.26 of [4]
% NK: was   fy_effect = (Rl./R0).^Q_FCY2 .* ( Q_FCY .* (Fy - Sfyg)./Fz0 ).^2;
% NK: In the paper, Q_FCY2 term goes inside the ( ).^2.  But it would be more
% NK: typical for it to be outside, as the form above: what does the
% NK: "equations manual" say?
            fy_effect = ( (Rl./R0).^Q_FCY2 .* Q_FCY .* (Fy - Sfyg)./Fz0 ).^2;
            ...
            fcorr = (1 + speed_effect - fx_effect - fy_effect) .* (1 + PFZ1 .* dpi);
            
            % Vertical force
            %Fz = fcorr.*(Q_FZ1.*(rho_z/R0) + Q_FZ2.*(rho_z/R0).^2).*Fz0;
            
            % Coefficients of quadratic in rho_z: Fz = KR1 * rho_z + KR2 * rho_z^2
            KR1 = fcorr.*Fz0./R0 * Q_FZ1;
            KR2 = fcorr.*Fz0./R0./R0 * Q_FZ2;
            
            Fz = KR1.*rho_z + KR2.*rho_z.^2;
        end % calculateFz62
    ...
    methods (Access = protected)
        ...
        function [forcesAndmoments, varinf] = doForcesAndMoments(obj, tirParams, postProInputs, internalParams, modes)%#codegen
            ...
            [Fy, Gyk, SVyk] = obj.calculateFy(tirParams, postProInputs, internalParams, modes, starVar, incrVar, Fy0, muy);
            
            [Mx] = obj.calculateMx(tirParams, postProInputs, incrVar, Fy);
 % NK: passed modes for limitsCheck test
            [My] = obj.calculateMy(tirParams, postProInputs, incrVar, Fx, modes);
            
            [Mz, t, Mzr] = obj.calculateMz(tirParams, postProInputs, internalParams, modes, starVar, primeVar, incrVar, alphar, alphat, Kxk, Kya_prime, Fy, Fx, Dr, Cr, Br, Dt, Ct, Bt, Et, Gyk, SVyk);
...
% NK: Added modes parameter (for useLimitsCheck)
        function [Re, omega, rho, Rl, a, b, Cx, Cy, Cz, sigmax, sigmay, instKya] = doExtras(obj, tirParams, postProInputs, forcesAndmoments, varinf, modes)%#codegen
            
% NK: Changed to handle MF5.2 NOMPRES == 0 
            pi0 = tirParams.NOMPRES;
            dpi = 0;
            if (pi0 > 0)
              dpi = (postProInputs.p - pi0)./pi0; % [Eqn (4.E2b) Page 177 - Book]
            end
            
            [Re, Romega, omega] = obj.calculateRe(tirParams, postProInputs, dpi);
            
            % Calculate the radius with the MF61 or MF62 formulation
% NK: Changed to call 62 only for 62 and above.
...
% NK: useLimitsCheck
            [a, b, ~] = obj.calculateContactPatch(tirParams, postProInputs, dpi, modes.useLimitsCheck);
        ...
        function [y] = interpolate(~, x0, y0, x1, y1, x)%#codegen
            %INTERPOLATE Interpolate between two points.
% NK: Slightly faster.  Also renamed with '0'
            y = y0 + (x - x0) ./ (x1 - x0) .* (y1 - y0);
% NK: was  %y = (yo.*(x1-x)+y1.*(x-xo))./(x1-xo);
...
% NK: Some handy functions defined outside the classdef (yes, you can do that!)
% NK: (if these cause problems with matlab code/compiler, then you can just
% NK: make them static methods by putting them in the methods(Static = true) section.
% NK: Functions declared outside the class seem to be faster than static functions.

% Used to avoid division-by-zero by adding a small value epsilon in direction of sign of x.
function x = make_nonzero(x, epsilon)
  x = x + (2.*(x >= 0) - 1) .* epsilon;
end

% Returns 1 if x >= 0, 0 if x < 0, as used throughout the book
function s = SGN(x)
  s = 2*(x >= 0) - 1;
end
